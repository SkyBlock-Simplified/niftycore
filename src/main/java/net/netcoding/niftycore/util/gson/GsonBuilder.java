...
import net.netcoding.niftycore.util.gson.internal.$Gson$Preconditions;
...
public final class GsonBuilder {
	...
	public GsonBuilder addDeserializationExclusionStrategy(
		...
		this.excluder = this.excluder.withExclusionStrategy(strategy, false,
				true);
		return this;
	}
	...
	public GsonBuilder addSerializationExclusionStrategy(
			ExclusionStrategy strategy) {
		this.excluder = this.excluder.withExclusionStrategy(strategy, true,
				false);
		return this;
	}
	...
	public Gson create() {
		...
		this.addTypeAdaptersForDate(this.datePattern, this.dateStyle,
				this.timeStyle, factories);

		return new Gson(this.excluder, this.fieldNamingPolicy,
				this.instanceCreators, this.serializeNulls,
				this.complexMapKeySerialization,
				this.generateNonExecutableJson, this.escapeHtmlChars,
				this.prettyPrinting, this.serializeSpecialFloatingPointValues,
				this.longSerializationPolicy, factories);
	}
	...
	public GsonBuilder registerTypeAdapter(Type type, Object typeAdapter) {
		...
		|| typeAdapter instanceof JsonDeserializer<?>) {
			...
			this.factories.add(TreeTypeAdapter.newFactoryWithMatchRawType(
					typeToken, typeAdapter));
		}
		if (typeAdapter instanceof TypeAdapter<?>)
			this.factories.add(TypeAdapters.newFactory(TypeToken.get(type),
	...
	public GsonBuilder registerTypeHierarchyAdapter(Class<?> baseType,
		...
		if (typeAdapter instanceof JsonDeserializer
			...
			this.hierarchyFactories.add(0, TreeTypeAdapter
					.newTypeHierarchyFactory(baseType, typeAdapter));
		if (typeAdapter instanceof TypeAdapter<?>)
			this.factories.add(TypeAdapters.newTypeHierarchyFactory(baseType,
					(TypeAdapter) typeAdapter));
		return this;
	}
