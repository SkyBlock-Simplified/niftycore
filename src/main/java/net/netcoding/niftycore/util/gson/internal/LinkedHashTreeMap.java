 ...
 * classes.
 */
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements
Serializable {
	/**
	...
	final class EntrySet extends AbstractSet<Entry<K, V>> {
		...
		public boolean contains(Object o) {
			return o instanceof Entry
					&& LinkedHashTreeMap.this.findByEntry((Entry<?, ?>) o) != null;
		}
		...
		public boolean remove(Object o) {
			...
			Node<K, V> node = LinkedHashTreeMap.this
					.findByEntry((Entry<?, ?>) o);
			if (node == null)
				return false;
	...
	static final class Node<K, V> implements Entry<K, V> {
		...
		Node(Node<K, V> parent, K key, int hash, Node<K, V> next,
				Node<K, V> prev) {
			this.parent = parent;
			this.key = key;
			this.hash = hash;
	...
	static <K, V> Node<K, V>[] doubleCapacity(Node<K, V>[] oldTable) {
		...
		for (int i = 0; i < oldCapacity; i++) {
			...
			newTable[i + oldCapacity] = rightSize > 0 ? rightBuilder.root()
					: null;
		}
		return newTable;
	}
	...
	public LinkedHashTreeMap(Comparator<? super K> comparator) {
		...
		this.threshold = (this.table.length / 2) + (this.table.length / 4); // 3/4
		// capacity
	}
