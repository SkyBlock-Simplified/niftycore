...
package net.netcoding.niftycore.util.gson.stream;
...
import net.netcoding.niftycore.util.gson.internal.JsonReaderInternalAccess;
...
public class JsonReader implements Closeable {
	...
	public void beginArray() throws IOException {
		...
		if (p == PEEKED_BEGIN_ARRAY) {
			...
			throw new IllegalStateException("Expected BEGIN_ARRAY but was "
					+ this.peek() + " at line " + this.getLineNumber()
					+ " column " + this.getColumnNumber() + " path "
					+ this.getPath());
	}
	...
	private void consumeNonExecutePrefix() throws IOException {
		...
		if (this.pos + NON_EXECUTE_PREFIX.length > this.limit
				&& !this.fillBuffer(NON_EXECUTE_PREFIX.length))
			return;
	...
	private int doPeek() throws IOException {
		...
		if (peekStack == JsonScope.EMPTY_ARRAY)
			...
			default:
				this.checkLenient();
				this.pos--; // Don't consume the first character in an unquoted
				// string.
	...
	public void endObject() throws IOException {
		...
		if (p == PEEKED_END_OBJECT) {
			this.stackSize--;
			this.pathNames[this.stackSize] = null; // Free the last path name so
													// that it
	...
	private boolean fillBuffer(int minimum) throws IOException {
		...
		this.pos = 0;
		int total;
		while ((total = this.in.read(buffer, this.limit, buffer.length
				- this.limit)) != -1) {
			this.limit += total;

			// if this is the first read, consume an optional byte order mark
			// (BOM) if it exists
			if (this.lineNumber == 0 && this.lineStart == 0 && this.limit > 0
					&& buffer[0] == '\ufeff') {
				this.pos++;
				this.lineStart++;
				minimum++;
	...
	public boolean nextBoolean() throws IOException {
		...
		throw new IllegalStateException("Expected a boolean but was "
				+ this.peek() + " at line " + this.getLineNumber() + " column "
				+ this.getColumnNumber() + " path " + this.getPath());
	}
	...
	public double nextDouble() throws IOException {
		...
		if (p == PEEKED_NUMBER) {
			this.peekedString = new String(this.buffer, this.pos,
					this.peekedNumberLength);
			this.pos += this.peekedNumberLength;
		} else if (p == PEEKED_SINGLE_QUOTED || p == PEEKED_DOUBLE_QUOTED)
			this.peekedString = this
		...
		double result = Double.parseDouble(this.peekedString); // don't catch
																// this
		...
		if (!this.lenient
			...
			throw new MalformedJsonException(
					"JSON forbids NaN and infinities: " + result + " at line "
							+ this.getLineNumber() + " column "
							+ this.getColumnNumber() + " path "
							+ this.getPath());
	...
	public int nextInt() throws IOException {
		...
		if (p == PEEKED_LONG) {
			...
			if (this.peekedLong != result)
				...
				throw new NumberFormatException("Expected an int but was "
						+ this.peekedLong + " at line " + this.getLineNumber()
						+ " column " + this.getColumnNumber() + " path "
						+ this.getPath());
			this.peeked = PEEKED_NONE;
			this.pathIndices[this.stackSize - 1]++;
		...
		this.peeked = PEEKED_BUFFERED;
		double asDouble = Double.parseDouble(this.peekedString); // don't catch
																	// this
		// NumberFormatException.
		result = (int) asDouble;
		if (result != asDouble)
			// 'int'.
			throw new NumberFormatException("Expected an int but was "
					+ this.peekedString + " at line " + this.getLineNumber()
					+ " column " + this.getColumnNumber() + " path "
					+ this.getPath());
		this.peekedString = null;
		this.peeked = PEEKED_NONE;
	...
	private int nextNonWhitespace(boolean throwOnEof) throws IOException {
		...
		while (true) {
			...
			if (c == '/') {
				this.pos = p;
				if (p == l) {
					this.pos--; // push back '/' so it's still in the buffer
								// when
					// this method returns
	...
	private void push(int newTop) {
		if (this.stackSize == this.stack.length) {
			...
			System.arraycopy(this.pathIndices, 0, newPathIndices, 0,
					this.stackSize);
	...
	private boolean skipTo(String toFind) throws IOException {
		outer: for (; this.pos + toFind.length() <= this.limit
				|| this.fillBuffer(toFind.length()); this.pos++) {
	...
	public String toString() {
		return this.getClass().getSimpleName() + " at line "
				+ this.getLineNumber() + " column " + this.getColumnNumber();
	}
}
